  ...
  flake.nixosModules.opencode =
    ...
    let
        ...
        image_gen = {
          ...
          command = [
            "${pkgs.writeShellScript "image-gen-mcp-wrapper" ''
              ...
              # Fallback remains the visual-engineering category selection.
              IMAGE_MODEL_CANDIDATE="$(${pkgs.jq}/bin/jq -r '
                first(
                  .providers
                  | to_entries[]
                  | .key as $provider
              ...
              if [ -n "$IMAGE_MODEL_CANDIDATE" ]; then
                export IMAGE_MODEL="$IMAGE_MODEL_CANDIDATE"
              else
                export IMAGE_MODEL="${state.categories."visual-engineering"}"
              fi
      ...
      opencodeMemConfig = {
        ...
        memoryModel = state.categories.deep;
