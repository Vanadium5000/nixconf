...
# Drop *.yaml/*.yml files below modules/terminal/docker/compose/<stack>/ and enable the stack here;
  ...
  flake.nixosModules.docker-compose-stacks =
    ...
    in
    {
      options.services.docker-compose-stacks = {
        ...
        stacks = mkOption {
          description = "Discovered Docker Compose stack directories under modules/terminal/docker/compose.";
          type = types.attrsOf (
            types.submodule {
              options = {
                enable = mkOption {
