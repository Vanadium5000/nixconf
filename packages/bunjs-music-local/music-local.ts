...
async function extractThumbnail(
  ...
  try {
    ...
    // -y: overwrite
    const res =
      await $`ffmpeg -y -v error -i ${inputFile} -an -vf scale=-1:120 -pix_fmt yuvj420p ${cacheFile}`.nothrow();
...
async function getItems(): Promise<MediaItem[]> {
  try {
    ...
    try {
      output =
        await $`mpc -f "%file%\t%artist%\t%title%" search filename ""`.text();
    } catch (e) {
...
async function showMenu(items: MediaItem[]): Promise<MediaItem | null> {
  ...
  items.forEach((item) => {
    ...
    const escapedTitle = (item.title || "")
      ...
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
    const escapedArtist = (item.artist || "")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
  ...
  try {
    ...
    const proc = Bun.spawn([menuCmd, "-p", "Local Music"], {
      stdin: "pipe",
      stdout: "pipe",
      env: {
        ...process.env,
...
async function main() {
  if (
    ...
    console.log(`Usage: qs-music-local

Browse the local MPD music library with qs-dmenu and play a selected track.`);
    return;
  }
