...
async function testGpgAgent(): Promise<TestResult[]> {
  ...
  const agentTest = await runCmd(["gpg-connect-agent", "/bye"]);
  results.push({
...
async function testPinentry(): Promise<TestResult[]> {
  ...
  results.push({
    ...
    message: dbusAddr
      ...
      : "Not set (some pinentry features may fail)",
    details: dbusAddr,
  });
...
async function showEnvironment(): Promise<void> {
  ...
  for (const v of vars) {
    ...
    if (value) {
      ...
      console.log(
        `  ${COLORS.dim}${v}=${COLORS.reset}${COLORS.yellow}(unset)${COLORS.reset}`,
      );
    }
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `${COLORS.bold}git-sync-debug${COLORS.reset} - Authentication diagnostics\n`,
  );
