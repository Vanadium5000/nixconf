...
async function showEditOptions(
 ...
 switch (selected) {
  ...
  case "":
   return "back";
  case "📋 Copy Credential Path": {
   const copyCmd = await getCopyCommand();
   await performAction(entryPath, "copy", copyCmd);
   await notify("Credential path copied to clipboard", "passmenu");
   return "back";
  }
...
async function main(): Promise<void> {
 ...
 const configuredPassDir = process.env.PASSWORD_STORE_DIR;
 const homeDirectory = process.env.HOME;
 // Some launchers preserve $HOME literally instead of expanding it. Resolve
 // that conventional value here so password-store discovery remains stable.
 const passDir = configuredPassDir?.startsWith("$HOME/")
  ? homeDirectory
   ? join(homeDirectory, configuredPassDir.slice("$HOME/".length))
   : configuredPassDir
  : (configuredPassDir || (homeDirectory ? join(homeDirectory, ".password-store") : ""));
 if (!passDir || !existsSync(passDir)) {
  const location = passDir || "$PASSWORD_STORE_DIR or $HOME/.password-store";
  console.error(`[passmenu] Password store directory is unavailable: ${location}`);
  await notify("Password store directory is unavailable", "passmenu");
  process.exit(1);
 }
