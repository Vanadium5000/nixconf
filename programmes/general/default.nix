...
}:
{
  perSystem =
    ...
    }:
    {
      ...
      packages.monero-wallet = inputs.wrappers.lib.wrapPackage {
        inherit pkgs;
        package =
          let
      ...
      packages.bitcoin-wallet = inputs.wrappers.lib.wrapPackage {
        inherit pkgs;
        package =
          let
            passCmd = "${(pkgs.pass.withExtensions (exts: [ exts.pass-otp ]))}/bin/pass";
      ...
      packages.litecoin-wallet = inputs.wrappers.lib.wrapPackage {
        inherit pkgs;
        package =
          let
            passCmd = "${(pkgs.pass.withExtensions (exts: [ exts.pass-otp ]))}/bin/pass";
      ...
      packages.ethereum-wallet = inputs.wrappers.lib.wrapPackage {
        inherit pkgs;
        package =
          let
            passCmd = "${(pkgs.pass.withExtensions (exts: [ exts.pass-otp ]))}/bin/pass";
