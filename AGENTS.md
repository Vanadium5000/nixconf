...
- **New Package**: nixpkgs package → `environment.systemPackages`; in-repo software → `packages/<name>/package.nix`; packaged upstream project → `external-packages/<name>/package.nix`; wrapper/configured upstream tool → `programmes/<name>/`; packages match `pname` and are exposed via `self.packages`. External packages must be covered in `external-packages/update-pkgs/workflow.nix` with an updater mode or documented manual reason, otherwise the coverage check and `update-pkgs` warn.
