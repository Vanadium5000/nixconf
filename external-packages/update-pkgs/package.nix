...
let
  ...
  workflow = import ./workflow.nix;
  shellCasePattern = values: builtins.concatStringsSep " | " values;
  shellCases =
    attrs:
    builtins.concatStringsSep "\n" (
      builtins.map (name: ''${name}) printf '%s\n' "${attrs.${name}}" ;;'') (builtins.attrNames attrs)
    );
  edgePackagePattern = shellCasePattern workflow.edgePackages;
  lightPackagePattern = shellCasePattern workflow.packageSets.light;
  ...
  coverageWarning =
    if missingCoverage == [ ] then
      ...
      builtins.warn "external-packages/update-pkgs: missing workflow coverage for ${builtins.concatStringsSep ", " missingCoverage}" "";
in
pkgs.writeShellApplication {
  ...
  text = ''
        : ${builtins.toString coverageWarning}

        set -e

        # Navigate to the external package directory
        if git rev-parse --show-toplevel >/dev/null 2>&1; then
          ROOT="$(git rev-parse --show-toplevel)"
          TARGET="$ROOT/external-packages"
          cd "$TARGET"
        else
    ...
    ${manualReasonCases}
        ...
        package_call_expr() {
          ...
          case "$pkg" in
          ${edgePackagePattern})
            printf '%s\n' "unstable.callPackage ./$pkg/package.nix {}"
            ;;
          *)
            printf '%s\n' "pkgs.callPackage ./$pkg/package.nix {}"
            ;;
          esac
        }

        package_files() {
          find . -mindepth 2 -maxdepth 2 -path "./*/package.nix" -printf "%P\n" | sort
        }

        package_names() {
          package_files | sed 's|/package\.nix$||'
        }
        ...
        package_file() {
          local pkg="$1"
          echo "$pkg/package.nix"
        }

        write_packages_expression() {
          {
            printf '%s\n' '{ pkgs ? import <nixpkgs> { config.allowUnfree = true; }, unstable ? import <nixpkgs-unstable> { config.allowUnfree = true; } }:'
            printf '%s\n' '{'
            local pkg
            while IFS= read -r pkg; do
              [ -z "$pkg" ] && continue
              printf '  %s = %s;\n' "$pkg" "$(package_call_expr "$pkg")"
            done < <(package_names)
            printf '%s\n' '}'
          } >packages.nix
        }

        warn_uncovered_packages() {
          local pkg uncovered=() mode
          while IFS= read -r pkg; do
            [ -z "$pkg" ] && continue
            mode=$(package_update_mode "$pkg")
            if [ "$mode" = nix-update+fallback ]; then
              uncovered+=("$pkg")
            fi
          done < <(package_names)

          if [ ''${#uncovered[@]} -gt 0 ]; then
            printf 'Warning: external packages without update-pkgs workflow coverage:\n' >&2
            printf '  - %s\n' "''${uncovered[@]}" >&2
          fi
        }
        ...
        run_flake_build_attr() {
          local pkg="$1"
          (cd "$ROOT" && nix build --no-link --print-out-paths "$(flake_attr "$pkg")")
        }

        run_package_build() {
          local pkg="$1"
          run_flake_build_attr "$pkg"
        }
        ...
        run_package_test() {
          ...
          main_program=$(
            (cd "$ROOT" && nix eval --raw "$(flake_attr "$pkg").meta.mainProgram") 2>/dev/null || true
          )
        ...
        refresh_fake_hash_from_build() {
          local file="$1" attr="$2"
          shift 2
          local old_hash output status new_hash
          old_hash=$(attr_hash "$file" "$attr")
          if [ -z "$old_hash" ]; then
            echo "    Could not find $attr in $file" >&2
            return 1
          fi

          set_attr_hash "$file" "$attr" "sha256-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
          set +e
          output=$("$@" 2>&1)
          status=$?
          set -e
          ...
          if [ -n "$new_hash" ] && is_sri_sha256 "$new_hash"; then
            set_attr_hash "$file" "$attr" "$new_hash"
            echo "    Updated $attr: $new_hash"
            return 0
          fi

          set_attr_hash "$file" "$attr" "$old_hash"
          printf '%s\n' "$output"
          echo "    Could not refresh $attr from build output (exit $status)." >&2
          return 1
        }
        ...
        update_omniroute_package() {
          ...
          # Source: external-packages/omniroute/package.nix
        ...
        update_cpa_usage_keeper_package() {
          ...
          echo "    Refreshing npmDepsHash..."
          if ! refresh_fake_hash_from_build "$file" npmDepsHash run_flake_build_attr cpa-usage-keeper-web; then
            return 1
          fi

          echo "    Refreshing vendorHash..."
          if ! refresh_fake_hash_from_build "$file" vendorHash run_flake_build_attr cpa-usage-keeper; then
            return 1
          fi
          ...
          if [ -z "$updater_id" ] || ! declare -F "$updater" >/dev/null; then
            ...
            nix-update-branch)
              ...
              if run_nix_update "$pkg" --version branch; then
                  UPDATED+=("$pkg")
          elif update_multi_source_package "$pkg"; then
                UPDATED+=("$pkg")
              else
              SKIPPED+=("$pkg")
              fi
            ...
            nix-update+fallback)
              ...
              ;;
            *)
          echo " Unknown update mode $(package_update_mode "$pkg") for $pkg"
                  FAILED+=("$pkg")
              ;;
            esac
          done
